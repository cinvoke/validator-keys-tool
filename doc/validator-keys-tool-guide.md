...
change if the casinocoind config and/or server are compromised.
...
*   sign tokens authorizing a casinocoind server to run as the validator identified
    by this public key.
...
The current token needs to be present in the casinocoind config file.
...
Sample output:
```
  Validator keys stored in /home/ubuntu/.casinocoin/validator-keys.json
```
...
```
  Update casinocoind.cfg file with these values:
...
For a new validator, add the [validator_token] value to the casinocoind config file.
...
value. After the config is updated, restart casinocoind.
...
```
  ...
  Update casinocoind.cfg file with these values and restart casinocoind:
...
restart casinocoind. Rename the old key file and generate new [validator keys](#validator-keys) and
